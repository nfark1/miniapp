...
const BOT_NAME = "auto59_bot"; // твой бот

// 🔹 Хелпер: приводит фото к корректному URL
const getPhotoUrl = (photo?: string | null) => {
  if (!photo) return "";
  // Полные ссылки (старые объявления с dev-доменом) — оставляем как есть
  if (photo.startsWith("http://") || photo.startsWith("https://")) {
    return photo;
  }
  // Если нет начального слэша — добавим
  if (!photo.startsWith("/")) {
    return "/" + photo;
  }
  // Обычный вариант: /uploads/xxx.jpeg
  return photo;
};
...
const shareListing = (item: Listing) => {
  ...
  const deepLink = `https://t.me/${BOT_NAME}?startapp=${encodeURIComponent(
    startParam,
  )}`;
  ...
  // Цена
  lines.push(`💰 ${item.price.toLocaleString("ru-RU")} ₽`);
  ...
  if (item.mileage != null) {
    lines.push(`🧭 ${item.mileage.toLocaleString("ru-RU")} км`);
  }
...
export const ItemDetail: React.FC<ItemDetailProps> = ({
  ...
  // 🔹 Нормализуем ссылки на фото
  const photos = (item.photos || []).map(getPhotoUrl);
        ...
        <div className="flex items-center gap-3 mr-2">
          ...
          <div className="flex items-center gap-1 text-[11px] text-slate-400">
            <svg
              ...
              stroke={
                isFav ? "rgba(34,211,238,1)" : "rgba(148,163,184,0.8)"
              }
              strokeWidth={1.8}
              strokeLinecap="round"
              strokeLinejoin="round"
